...
Friend Class frmPayrollDistribution
    Public State As FormState
    Public PK As Double

    Dim blnUpdateInProgress As Boolean
    Dim sSQL As String
    Private Sub frmPayrollDistribution_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        Call FillListview("SELECT * FROM tblMst_PayrollDistribution ORDER BY PayrollDistributionCode ASC ", lvList)
        ...
        Select Case srcPerformWhat
            ...
            Case "Save"
                ...
                If State = FormState.AddStateMode Then
                    PK = GenerateNewIDNumber("tblMst_PayrollDistribution", "PayrollDistributionCode")

                    sSQL = "INSERT INTO tblMst_PayrollDistribution"
                    sSQL = sSQL & "(PayrollDistributionCode"
                    sSQL = sSQL & ", Description"
                    sSQL = sSQL & ", WorkingDays"
                    sSQL = sSQL & ", PayrollFrequency"
            ...
            Case "Delete"
                ...
                If MsgBox("Are you sure you want to delete selected record? <Y/N>", MsgBoxStyle.Critical + MsgBoxStyle.YesNo) = MsgBoxResult.Yes Then
                    Call DeleteSQL("tblMst_PayrollDistribution", "PayrollDistributionCode", True, "", PK)
                Else
                    Exit Sub
                End If
...
ErrHandler: MsgBox(Err.Description, vbExclamation)
    ...
    Public Sub FillFormView(ByVal p_k As Double)
        ...
        Dim SQLCmd As New SqlCommand("SELECT * FROM tblMst_PayrollDistribution WHERE PayrollDistributionCode=" & toNumber(p_k), MyConn)
        ...
        If Dtrdr.Read = False Then
            Exit Sub
        Else
            PK = Dtrdr.Item("PayrollDistributionCode").ToString

            txtID.Text = Dtrdr.Item("PayrollDistributionCode").ToString
            txtDescription.Text = Dtrdr.Item("Description").ToString
            txtDays.Text = Dtrdr.Item("WorkingDays").ToString
            txtFrequency.Text = Dtrdr.Item("PayrollFrequency").ToString
...
ErrHandler:
    ...
    Private Sub FillListview(ByVal sSQL As String, ByRef srcListview As ListView)
        ...
        Do While Dtrdr.Read()
            Dim lv As ListViewItem = srcListview.Items.Add(Dtrdr.Item("PayrollDistributionCode").ToString())
            lv.SubItems.Add(Dtrdr.Item("Description").ToString())
            lv.SubItems.Add(Dtrdr.Item("WorkingDays").ToString())
            lv.SubItems.Add(Dtrdr.Item("PayrollFrequency").ToString())
        Loop
        Dtrdr.Close()
        SQL.Dispose()
        MyConn.Close()
...
Mars:   MsgBox(Err.Description, MsgBoxStyle.Critical)
    ...
    Private Sub btnFind_Click(sender As Object, e As EventArgs) Handles btnFind.Click
        ...
        If txtFind.Text = "" Then
            Call FillListview("SELECT * FROM tblMst_PayrollDistribution ORDER BY PayrollDistributionCode ASC ", lvList)
        Else
            Call FillListview("SELECT * FROM tblMst_PayrollDistribution WHERE " &
                          " Description LIKE '%" & sqlSafe(txtFind.Text) & "%' " &
                          " OR WorkingDays LIKE '%" & sqlSafe(txtFind.Text) & "%'", lvList)
        End If
        Exit Sub
ErrFinder:
        MsgBox(Err.Description, MsgBoxStyle.Critical)
