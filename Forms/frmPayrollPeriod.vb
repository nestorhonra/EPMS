...
Friend Class frmPayrollPeriod
    ...
    Private Sub frmPayrollPeriod_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        Call FillListview("SELECT * FROM tblMst_PayrollPeriod ORDER BY PeriodCode ASC ", lvList)
    ...
    Public Sub CommandPass(ByVal srcPerformWhat As String)
        ...
        Select Case srcPerformWhat
            Case "New"
                ...
                txtDescription.Text = UCase(MonthName(Month(GetServerDateTime)))
                txtYear.Text = Year(GetServerDateTime)
            ...
            Case "Save"
                ...
                If State = FormState.AddStateMode Then
                    PK = GenerateNewIDNumber("tblMst_PayrollPeriod", "PeriodCode")

                    sSQL = "INSERT INTO tblMst_PayrollPeriod"
                    sSQL = sSQL & "(PeriodCode"
                    sSQL = sSQL & ", MonthText"
                    sSQL = sSQL & ", YearText"
                    sSQL = sSQL & ", StartDate"
                    sSQL = sSQL & ", EndDate"
                    ...
                    sSQL = sSQL & ", '" & sqlSafe(UCase(txtYear.Text)) & "'"
                    sSQL = sSQL & ", '" & CDate(dtFrom.Value) & "'"
                    sSQL = sSQL & ", '" & CDate(dtTo.Value) & "'"
                    sSQL = sSQL & ", 'N'"
                    sSQL = sSQL & ", '" & CDate(GetServerDateTime()) & "'"
                    sSQL = sSQL & ", '" & ACTIVE_USER.m_LoginName & "')"

                ElseIf State = FormState.EditStateMode Then
                    sSQL = "UPDATE tblMst_PayrollPeriod SET"
                    sSQL = sSQL & " MonthText = '" & sqlSafe(UCase(txtDescription.Text)) & "'"
                    sSQL = sSQL & " ,YearText = '" & sqlSafe(UCase(txtYear.Text)) & "'"
                    sSQL = sSQL & " ,StartDate = '" & CDate(dtFrom.Value) & "'"
                    sSQL = sSQL & " ,EndDate = '" & CDate(dtTo.Value) & "'"
                    sSQL = sSQL & " ,LastDateModified = '" & CDate(GetServerDateTime()) & "'"
                    sSQL = sSQL & " ,ModifiedBy = '" & ACTIVE_USER.m_LoginName & "'"
                    sSQL = sSQL & " WHERE PeriodCode = " & toNumber(PK)
                End If
            ...
            Case "Delete"
                ...
                If MsgBox("Are you sure you want to delete selected record? <Y/N>", MsgBoxStyle.Critical + MsgBoxStyle.YesNo) = MsgBoxResult.Yes Then
                    Call DeleteSQL("tblMst_PayrollPeriod", "PeriodCode", True, "", PK)
                Else
                    Exit Sub
                End If
...
ErrHandler: MsgBox(Err.Description, vbExclamation)
    ...
    Public Sub FillFormView(ByVal p_k As Double)
        ...
        Dim SQLCmd As New SqlCommand("SELECT * FROM tblMst_PayrollPeriod WHERE PeriodCode=" & toNumber(p_k), MyConn)
        ...
        If Dtrdr.Read = False Then
            Exit Sub
        Else
            PK = Dtrdr.Item("PeriodCode").ToString

            txtID.Text = Dtrdr.Item("PeriodCode").ToString
            txtDescription.Text = Dtrdr.Item("MonthText").ToString
            dtFrom.Value = FormatDateTime(Dtrdr.Item("StartDate").ToString, DateFormat.ShortDate)
            dtTo.Value = FormatDateTime(Dtrdr.Item("EndDate").ToString, DateFormat.ShortDate)
            CheckBox1.Checked = changeYNValue(Dtrdr.Item("IsPosted").ToString)
            txtYear.Text = Dtrdr.Item("YearText").ToString
...
ErrHandler:
    ...
    Private Sub FillListview(ByVal sSQL As String, ByRef srcListview As ListView)
        ...
        Do While Dtrdr.Read()
            Dim lv As ListViewItem = srcListview.Items.Add(Dtrdr.Item("PeriodCode").ToString())
            lv.SubItems.Add(Dtrdr.Item("MonthText").ToString())
            lv.SubItems.Add(Dtrdr.Item("YearText").ToString())
...
Mars:   MsgBox(Err.Description, MsgBoxStyle.Critical)
    ...
    Private Sub btnFind_Click(sender As Object, e As EventArgs) Handles btnFind.Click
        ...
        If txtFind.Text = "" Then
            Call FillListview("SELECT * FROM tblMst_PayrollPeriod ORDER BY PeriodCode ASC ", lvList)
        Else
            Call FillListview("SELECT * FROM tblMst_PayrollPeriod WHERE " &
                          " MonthText LIKE '%" & sqlSafe(txtFind.Text) & "%' " &
                          " OR YearText LIKE '%" & sqlSafe(txtFind.Text) & "%'", lvList)
        End If
        Exit Sub
ErrFinder:
        MsgBox(Err.Description, MsgBoxStyle.Critical)
    ...
    Private Sub frmPayrollPeriod_FormClosed(sender As Object, e As FormClosedEventArgs) Handles Me.FormClosed
        Me.Dispose()
    End Sub
